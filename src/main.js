...
const feed = {
  ...
  getters: {
    getPublishedContent: state => {
      return state.publishedContent
    },
    getUsers: state => {
      return state.users
    },
    ...
    getUserPosts: (state) => (username) => {
      return state.publishedContent.filter(post => post.username === username)
    },
    getActiveUser: state => {
    ...
    getActiveUserInfo:state => {
      return state.users.find(user => user.username === state.activeUser)
    }
  },
  actions: {
    ...
    editPost({commit}, editInfo){
      commit('edit', editInfo)
    },
    changeDarkmode({commit}, value){
      commit('changeTheme', value)
    },
    changeActiveUser({commit}, username){
      commit('changeUser', username)
    }
  },
  mutations: {
    post(state, postData){
      ...
      state.publishedContent.unshift(newPost)
      state.nextId++
    },
    clear(state){
      state.publishedContent = state.publishedContent.filter(post => post.username !== state.activeUser)
    },
    delete(state, id){
      const index = state.publishedContent.findIndex((element) => element.username === state.activeUser && element.id === id)
      if(index !== -1){
        state.publishedContent.splice(index, 1)
      }
    },
    edit(state, {id, newText}){
      const index = state.publishedContent.findIndex((element) => element.username === state.activeUser && element.id === id)
      if(index !== -1){
        state.publishedContent[index].text = newText
      }
    },
...
router.beforeEach((to, from, next) => {
  ...
  if(to.fullPath === '/feed/kenadams' ) next('/')
  else next()
})
new Vue({
  vuetify,
